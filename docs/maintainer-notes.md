...
Use one version value in each of these forms for a release such as 1.9.0:

| Surface | Required form |
|---|---|
| Release branch | `release/1.9.0` |
| Composer/Packagist version | `1.9.0` |
| Git tag | `v1.9.0` |
| GitHub release title | `v1.9.0` |
...
the Composer/Packagist release metadata to `1.9.0` in the release commit so
Packagist presents that version even though the Git tag is `v1.9.0`.

## Release workflow

1. Create `release/<version>` from the default branch (`master` or `main`),
   for example `release/1.9.0`.
